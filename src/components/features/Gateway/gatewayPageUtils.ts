...
export const buildGatewayConnectHost = (host: string, localOnly: boolean): string => {
  ...
  if (value === '0.0.0.0' || value === '::') {
    return localOnly ? '127.0.0.1' : '<LAN IP>'
  }
    return localOnly ? '127.0.0.1' : '<LAN IP>'
  }
...
export const createGatewayFieldErrors = (config: any) => {
  ...
  if (!host) {
    errors.host = 'Listen address cannot be empty'
  } else if (!isValidGatewayHost(host)) {
    errors.host = 'Listen address must be localhost, IPv4, or IPv6 address'
  }
  ...
  if (!region) {
    ...
    errors.region = `region is not supported: ${region}`
  }

  if (!['single', 'group', 'pool'].includes(accountMode)) {
    errors.accountMode = 'accountMode must be single/group/pool'
  } else if (accountMode === 'single' && !String(config?.accountId || '').trim()) {
    errors.accountId = 'single mode requires an account'
  ...
  if (!localOnly && !allowedIps.length) {
    errors.allowedIpsText = 'At least one allowlisted source IP is required when remote access is enabled'
  }
...
export const buildGatewayRoutingSummary = ({ config, counts, selectedLabels = {} }: any) => {
  ...
  if (mode === 'single') {
    return {
      ...
      modeDescription: 'The proxy always uses one account, suitable for debugging or single-tenant binding.',
  ...
  if (mode === 'group') {
    return {
      ...
      modeDescription: 'Lock an account group first, then select available accounts by strategy and threshold.',
  ...
  if (mode === 'pool') {
    return {
      ...
      modeDescription: 'Use all available accounts and automatically select by strategy and threshold without group restrictions.',
...
export const buildGatewayActionSummary = ({
  ...
  if (hasFieldErrors) {
    return {
      ...
      description: 'The current form has invalid configuration. Save, start, and restart are blocked until highlighted fields are fixed.'}
  }

  if (running && unsavedChanges && runtimeChanges) {
    return {
      tone: 'yellow',
      title: 'Configuration changed, restart to apply',
      description: 'The proxy is still running with its startup configuration. Save first, then restart the proxy to apply changes.'}
  }

  if (running && unsavedChanges) {
    return {
      tone: 'blue',
      title: 'Current running configuration is not saved',
      description: 'Current page configuration is used by the running proxy but has not been written back. Save it to keep it for next startup.'}
  }
  ...
  if (running) {
    return {
      tone: 'teal',
      title: 'Proxy running',
      description: 'Current configuration matches saved state. Stop the proxy to interrupt traffic.'}
  }

  if (unsavedChanges) {
    return {
      tone: 'blue',
      title: 'Can start with current configuration',
      description: 'Starting the proxy will use the current form configuration. Save first if you want these settings reused on next app launch.'}
  }

  return {
    tone: 'blue',
    title: 'Proxy is not running',
    description: 'Start the existing configuration directly, or adjust the form first.'}
}
...
export const buildGatewaySecuritySummary = ({ config }: any) => {
  ...
  return {
    exposureLabel: config?.localOnly ? 'Local access only' : 'Remote access allowed',
    allowedIpsCount,
...
export const buildGatewayIntegrationSummary = ({ baseUrl, apiKey, clientApiKeysText, logDir, errorHistory }: any) => {
  ...
  return {
    ...
    logDirState: String(logDir || '').trim() ? 'Log directory located' : 'Log directory not retrieved',
...
export const buildGatewayMetricsSummary = (entries: any) => {
  ...
  logs.forEach(item => {
    ...
    const model = String(item?.model || 'Model not recorded').trim() || 'Model not recorded'
    modelCounts[model] = (modelCounts[model] || 0) + 1

    const upstream = String(item?.upstreamSource || 'Upstream source unresolved').trim() || 'Upstream source unresolved'

...
function HooksPanel({ onCountChange, projectDir }: any) {
  ...
  const loadHooks = useCallback(async () => {
    ...
    try {
      ...
      handleUiError('Failed to load Hooks', e, { userMessage: t('hooks.loadFailed') || 'Failed to load Hooks' })
    } finally {
      setLoading(false)
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      handleUiError(t('hooks.saveFailedInternal'), e, { userMessage: t('hooks.saveFailed') || t('hooks.saveFailedInternal') })
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDelete = async (hookFile: any) => {
    ...
    try {
      ...
      handleUiError('Failed to delete Hook', e, { userMessage: t('hooks.deleteFailed') || 'Delete failed' })
    }
  }
...
`
    try {
      ...
      handleUiError(t('hooks.createFailedInternal'), e, { userMessage: t('hooks.createFailed') || t('hooks.createFailedInternal') })
      return false
    }
  }

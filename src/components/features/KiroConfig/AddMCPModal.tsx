...
function AddMCPModal({ onClose, onSuccess, projectDir }: any) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setParseResult({ servers: [], error: 'Invalid JSON format' })
      setDuplicates([])
    }
  ...
  const handleSave = async () => {
    if (!parseResult || parseResult.servers.length === 0) {
      setError(parseResult?.error || 'No valid configuration')
      return
    }
    ...
    if (results.failed.length > 0) {
      ...
      const summary = [`Partial failure: ${failedNames}`]
    ...
    if (results.success.length === 0 && results.skipped.length > 0) {
      setError(`No services added, skipped: ${results.skipped.join(', ')}`)
      return
    }

    const summaryParts = []
    if (results.success.length > 0) summaryParts.push(`${results.success.length} added`)
    if (results.skipped.length > 0) summaryParts.push(`${results.skipped.length} skipped`)
    if (results.renamed.length > 0) summaryParts.push(`${results.renamed.length} renamed`)
  ...
  const displayNames = serverNames.length > 3 
    ? serverNames.slice(0, 3).join(', ') + ` and ${serverNames.length} more`
    : serverNames.join(', ')
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="relative flex-1 overflow-auto p-6 space-y-4">
          {/* 快速模板 */}
          <div>
            <label className={`block text-xs text-muted-foreground mb-1.5`}>Quick fill</label>
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-1.5">
              <div className="flex items-center gap-2 flex-1 min-w-0">
                <label className={`text-xs text-muted-foreground shrink-0`}>Config</label>
            ...
            <div className={`text-xs text-muted-foreground mt-1.5`}>
              Supports {`{ "name": config }`} or {`{ "mcpServers": { ... } }`} format
            </div>
          </div>
          ...
          {duplicates.length > 0 && (
            <div className="text-xs text-amber-500 bg-amber-500/10 px-3 py-2 rounded-lg flex flex-col gap-2">
              <div className="flex items-start gap-2">
                ...
                <span>
                  Existing services: {duplicates.join(', ')}
                  {duplicateStrategy === 'overwrite' && ' (will be overwritten)'}
                  {duplicateStrategy === 'skip' && ' (will be skipped)'}
                  {duplicateStrategy === 'rename' && ' (will be auto-renamed)'}
                </span>
              </div>
        ...
        <div className={`relative flex justify-end gap-3 px-6 py-4 border-t border-border`}>
          ...
          >
            {saving ? t('common.saving') : serverCount > 1 ? `Add ${serverCount}` : t('common.add')}
          </button>
        </div>
      </div>
    </div>
